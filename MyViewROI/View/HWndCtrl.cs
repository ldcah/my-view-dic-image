...
namespace MyViewROI
{
    ...
    public class HWndCtrl
    {
        ...
        private List<Txt> txtList=new List<Txt> ();
        /// <summary>
        ...
        public void repaint(HalconDotNet.HWindow window)
        {
            ...
            foreach (var entry in HObjList)
            {
                ...
                window.SetColor(entry.SetColor);
                window.SetDraw(entry.SetDraw);
                window.DispObj(entry.HObj);
            }

            foreach(var v in txtList)
            {
                mGC.applyContext(window, v.gContext);               
                //window.SetTposition(v.Row, v.Col);
                //window.SetColor(v.SetColor);
                window.SetFont(v.Font);//字体大小
                window.DispText(v.MSG, "image", v.Row, v.Col, v.SetColor, "box", "false");
                //window.WriteString(v.MSG);
            }
        ...
        public void addIconicVar(HObject obj,string setColor = "red",string setDraw= "margin")
        {
            ...
            if (obj == null)
                return;
                if(obj.GetType().Equals(typeof(HImage)))
                {
                    HImage image = (HImage)obj;
                        ...
                        Image = image;
                        var area = image.GetDomain().AreaCenter(out  _, out _);
                        image.GetImagePointer1(out _, out  newImageWidth, out  newImageHeight);
        ...
        {
            Txt entry = new Txt(msg, mGC.copyContextList(), row, col, setColor: setColor,font: setFont);
            txtList.Add(entry);
            //repaint();
        }

        
         HTuple newImageWidth, newImageHeight,_;
        public void addHoImage(HObject hoImage)
        {

            //if (hoImage==null|| !hoImage.IsInitialized()||hoImage.Key == IntPtr.Zero)
            //{
            //    return;
            //}


            HObjList.RemoveAll(v => v.ObjType == "Image");//删除之前的图像文件
            ...
            HOperatorSet.GetImageSize(hoImage, out  newImageWidth, out  newImageHeight);
            SetNewPart(newImageWidth, newImageHeight);             
            HObjList.Add(new HObjectEntry(hoImage, mGC.copyContextList(), objType: "Image"));
            }

            repaint();
        }

        public void SetNewPart(int newImageWidth, int newImageHeight)
        {
        ...
        {
            foreach(var v  in HObjList)//2021-10-21 销毁
            {
                if (v.ObjType != "Image") v.HObj.Dispose();
            }
            HObjList.Clear();
        ...
        public void clearTempRegionMsgWithOutImage()
        {
            ...
            var ieRegion = (from v in HObjList
                     where v.ObjType != "Image"
                     select v).ToList();

            foreach(var v in ieRegion)
            {
                v.HObj.Dispose();
                HObjList.Remove(v);
            }
        ...
        /// <summary>
        /// 定位到坐标点  2020-01-07 ludc
        /// </summary>
        /// <param name="colX"></param>
        /// <param name="rowY"></param>
        public void DispLocationHObject2XY(double colX, double rowY, string des)
        {

            HTuple row1, col1, row2, col2;
            //returns the upper left and lower right corner of the image part shown in the window
            HOperatorSet.GetPart(viewPort.HalconWindow, out row1, out col1, out row2, out col2);
            //获取窗体中图像 半长 半宽
            double imagePartInWindHalfHeight = (row2 - row1) / 2;
            double imagePartInWindHalfWidth = (col2 - col1) / 2;


            //设置
            ImgRow1 = rowY - imagePartInWindHalfHeight;
            ImgRow2 = rowY + imagePartInWindHalfHeight;
            ImgCol1 = colX - imagePartInWindHalfWidth;
            ImgCol2 = colX + imagePartInWindHalfWidth;
        ...
        /// <summary>
        /// 定位工具对象 2020-01-07  ludc
        /// </summary>
        private HObject LocationHObjectWithColor = null;
        /// <summary>
        /// 定位到坐标点  2020-01-07 ludc
        /// </summary>
        /// <param name="colX"></param>
        /// <param name="rowY"></param>
        public void DispLocationHObject2XY(double colX, double rowY )
        {

            //销毁+产生定位区域
            if (LocationHObjectWithColor != null && LocationHObjectWithColor.IsInitialized())
            {
                LocationHObjectWithColor.Dispose();
            }
            HOperatorSet.GenCircle(out LocationHObjectWithColor, rowY, colX, 100);


            HObjList.RemoveAll(v => v.ObjType == "Location");//删除之前的图像文件

            HObjList.Add(new HObjectEntry(LocationHObjectWithColor, mGC.copyContextList(), objType: "Location", setColor: "yellow"));

            DispLocation2XY(colX, rowY);
        }


        public void DispLocation2XY(double colX, double rowY)
        {
            HTuple row1, col1, row2, col2;
            //returns the upper left and lower right corner of the image part shown in the window
            HOperatorSet.GetPart(viewPort.HalconWindow, out row1, out col1, out row2, out col2);
