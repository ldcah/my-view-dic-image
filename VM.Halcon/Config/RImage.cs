...
namespace VM.Halcon.Config
{
    ...
    public class RImage : HImage, ISerializable
    {
        ...
        #region 构造函数
        public RImage() : base() { }
        public RImage(int dispViewID) : base() { DispViewID = dispViewID; }
        ...
        public void RemoveHRoi(string moduleName)
        {
            mHRoi.RemoveAll(e => e.ModuleName == moduleName);
        }
        public void RemoveHText(string moduleName)
        {
            mHText.RemoveAll(e => e.ModuleName == moduleName);
        }
        ...
        public void ShowHRoi(HRoi ROI)
        {
            try
            {
                int index = mHRoi.FindIndex(e => e.ModuleEncode == ROI.ModuleEncode && e.roiType == ROI.roiType && e.ModuleName == ROI.ModuleName);
                if (ROI.fors == true)
                {
                    mHRoi.Add(ROI);
                    return;
        ...
        public void ShowHText(HText ROI)
        {
            try
            {
                int index = mHText.FindIndex(e => e.ModuleName == ROI.ModuleName && e.roiType == ROI.roiType && e.ModuleName == ROI.ModuleName);
                if (ROI.fors == true)
                {
                    mHText.Add(ROI);
                    return;
