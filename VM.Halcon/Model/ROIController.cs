...
namespace VM.Halcon.Model
{
    ...
    public class ROIController
    {
        ...
        {
            double zoomScale = 1 / viewController.ZoomWndFactor; //根据图像缩放系数计算缩放率
            double scale = (double)viewController.ImageWidth/5488;  // 5488：ROI中心矩形和拖拽矩形设置尺寸时的基准图像宽度
            double size = scale * 20; //  20：在基准图像大小下选取的合适尺寸
            ...
            if (ROIList.Count > 0)
            {
                ...
                foreach (KeyValuePair<string, ROI> kvp in ROIList)
                {
                    ...
                    //  ROI中心矩形和拖拽矩形可根据图像缩放比例自适应大小
                    if (zoomScale <= 1)
                    {
                        //kvp.Value.m_Size = size * zoomScale;
                        kvp.Value.MoveRoiSize = (int)(20 * zoomScale);
                        kvp.Value.m_Size = size;
                        kvp.Value.m_Scale = zoomScale;
                    }
                    else
                    {
                        kvp.Value.MoveRoiSize = (int)(20 / zoomScale);
        ...
        /// <param name="rois"></param>
        protected internal void genRect1(string name, string color, double row1, double col1, double row2, double col2, ref Dictionary<string, ROI> rois)
        {
            SetROIShape(new ROIRectangle1());
            if (rois == null)
            {
        ...
        /// <param name="length2"></param>
        /// <param name="rois"></param>
        protected internal void genRect2(string name, string color, double row, double col, double phi, double length1, double length2, ref Dictionary<string, ROI> rois)
        {
            SetROIShape(new ROIRectangle2());
        ...
        /// <param name="radius"></param>
        /// <param name="rois"></param>
        protected internal void genCircle(string name, string color, double row, double col, double radius, ref Dictionary<string, ROI> rois)
        {
            SetROIShape(new ROICircle());
            if (rois == null)
            {
        ...
        /// <param name="radius"></param>
        /// <param name="rois"></param>
        protected internal void genCircleAre(string name, string color, double row, double col, double radius, ref Dictionary<string, ROI> rois)
        {
        ...
        /// <param name="endCol"></param>
        /// <param name="rois"></param>
        protected internal void genLine(string name, string color, double beginRow, double beginCol, double endRow, double endCol, ref Dictionary<string, ROI> rois)
        {
            this.SetROIShape(new ROILine());
        ...
        /// <param name="endCol"></param>
        /// <param name="rois"></param>
        protected internal void genCoordLine(string name, string color, double beginRow, double beginCol, double endRow, double endCol, ref Dictionary<string, ROI> rois)
        {
        ...
        /// <param name="radius"></param>
        /// <param name="rois"></param>
        protected internal void genPoint(string name, string color, double row, double col, ref Dictionary<string, ROI> rois)
        {
            SetROIShape(new ROIPoint());
            if (rois == null)
            {
