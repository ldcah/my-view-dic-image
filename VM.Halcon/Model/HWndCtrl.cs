...
namespace VM.Halcon.Model
{
    ...
    public class HWndCtrl
    {
        ...
        public int WindowWidth, WindowHeight, ImageWidth, ImageHeight, PrevCompX, PrevCompY;
        ...
        public double ZoomWndFactor, ZoomAddOn;
        ...
        private void MouseDown(object sender, HMouseEventArgs e)
        {
            ...
            if (drawModel) return;
            Repaint();
            ViewPort.Cursor = System.Windows.Forms.Cursors.NoMove2D;
        ...
        public void DispObj(HObject hObj, string color,bool isFillDisp)
        {
            lock (this)
            {
                try
                {
                   // HOperatorSet.SetLineWidth(ViewPort.HalconWindow, 5);
                       
                    //显示指定的颜色
                    if (color == null || color == "red")
                    {
                        HOperatorSet.SetColor(ViewPort.HalconWindow, "red");
                        ViewPort.HalconWindow.SetRgba(255, 0, 0, 100);
                    }
                    else
                    {
        ...
        private void ShowHObjectList()
        {
            try
            {
                foreach (HObjectWithColor hObjectWithColor in hObjectList)
                {
                    if (hObjectWithColor.Color == null || hObjectWithColor.Color == "red")
                    {
                        ViewPort.HalconWindow.SetRgba(255, 0, 0, 100);
                    }
                    else
